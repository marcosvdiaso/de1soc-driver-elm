...
/*
Função para iniciação de mouse + desnenho
Trackeia a posição do mouse + cliques por meio da struct input_event
Registra clamp para impedir que mouse saia da área delimitada
Acumula movimento e faz o polling dos botões (para poder fazer a troca de modo, entre desenho/apagar/nada)
Após acumular movimento e registrar cliques, faz a atualização na tela no fim da sincronização
A cada sincronização também atualiza o vetor
 */
